...
public:
  source(std::ostream& os, bool layout = true, bool midi = true)
  : os(os)
  , layout(layout)
  , midi(midi)
  {
...
//[ ::music::braille::ambiguous::sign
  void operator() (braille::ambiguous::barline const&) const
  {
  }
  void operator() (braille::ambiguous::simile const&) const
  { BOOST_ASSERT(false); }
...
private: // utilities
  ...
  void ly_partial(rational const& duration) const
  {
    os << "\\partial" << " " << duration.denominator();
    if (duration.numerator() != 1) os << "*" << duration.numerator();
    os << " ";
  }
  void ly_pitch_step(diatonic_step step) const
  {
...
, music::braille::ambiguous::score const& score
  ...
  case music::output_format::lilypond: {
    music::lilypond::source generate(stream);
    generate(score);
    break;
  }
  default:
